	...
	login: async (req, res) => {
		try {
			...
			const convertedPhone =
				account.charAt(0) === '0' ? account.slice(1) : account.substring(0, 3) === '+63' ? account.slice(3) : account;
			...
			if (!isMatchPassWord) return res.status(400).json({ msg: 'Wrong Credentials/Password' });
