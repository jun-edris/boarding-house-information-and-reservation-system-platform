...
const RegisterForm = () => {
	...
	const city = (code) => {
		axios
			.get(`https://psgc.gitlab.io/api/provinces/${code.target.value}/cities-municipalities`)
			.then(({ data }) => {
				setCity(data);
			})
			.catch((error) => {
	...
	const barangay = (code) => {
		axios
			.get(`https://psgc.gitlab.io/api/cities-municipalities/${code.target.value}/barangays`)
			.then(({ data }) => {
				setBarangay(data);
			})
			.catch((error) => {
		...
		axios
			.get(`https://psgc.gitlab.io/api/cities-municipalities/${code.target.value}`)
			.then(({ data }) => {
				setCit(data.name);
			})
			.catch((error) => {
		...
		<>
			<Formik
				...
				validationSchema={Yup.object().shape({
					...
					middleName: Yup.string().max(255).required('The middle name field is required'),
					...
					province: Yup.string().max(255).required('The region field is required'),
					city: Yup.string().max(255).required('The region field is required'),
					barangay: Yup.string().max(255).required('The region field is required'),
					contact: Yup.string()
						.matches(/^(9)/, 'Must start with 9')
						.max(10, 'Must be 10 digits')
						.required('Contact number is required'),
					password: Yup.string().min(8, 'At least 8 characters!').max(255).required('The password field is required'),
					confirmPass: Yup.string()
						.oneOf([Yup.ref('password'), null], 'Passwords must match')
						.required('Confirm your password'),
					role: Yup.string().oneOf(['landlord', 'tenant'], 'Invalid user role').required('Please choose a role'),
				})}
				onSubmit={async (values, { setErrors, setStatus, setSubmitting, resetForm }) => {
					try {
						setSubmitting(true);
						const { firstName, middleName, lastName, email, contact, password, role } = values;
			...
			>
				{({ errors, handleBlur, handleChange, handleSubmit, isSubmitting, touched, values }) => (
					<form noValidate onSubmit={handleSubmit}>
						<Grid container spacing={2} alignItems='center' direction={{ xs: 'column-reverse', lg: 'row' }}>
							<Grid item xs={12} lg={8}>
								<Grid container spacing={2}>
									<Grid item xs={12}>
										...
										{Boolean(touched.role && errors.role) && <FormHelperText error>{errors.role}</FormHelperText>}
									</Grid>
									<Grid item xs={12}>
										<Grid container spacing={2}>
							...
							<Grid item xs={12} lg={4} justifyContent='center'>
								<Box display='flex' alignItems='center' justifyContent='center' flexDirection='column'>
									<Box sx={{ position: 'relative' }}>
										<Avatar variant='rounded' sx={{ width: 150, height: 150, borderRadius: 5 }}>
											{preview ? (
												<img src={preview} alt='To upload' width='150' height='150' aspectRatio={1 / 1} />
											) : (
										...
										<ButtonUploadWrapper>
											...
											/>
											<label htmlFor='image'>
												<Button variant='contained' sx={{ borderRadius: '100%', py: 2.1 }} component='span'>
						...
						<Grid container spacing={2} sx={{ mt: 1 }}>
							<Grid item xs={12} sm>
								...
								{Boolean(touched.region && errors.region) && <FormHelperText error>{errors.region}</FormHelperText>}
							</Grid>
							<Grid item xs={12} sm>
								<FormControl fullWidth variant='filled'>
							...
							<Grid item xs={12} sm>
								...
								{Boolean(touched.city && errors.city) && <FormHelperText error>{errors.city}</FormHelperText>}
							</Grid>
							<Grid item xs={12} sm>
								<FormControl fullWidth variant='filled'>
