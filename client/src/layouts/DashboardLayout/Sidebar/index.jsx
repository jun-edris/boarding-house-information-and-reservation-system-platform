		...
		<Drawer variant='permanent' open={open} key={index}>
			...
			>
				<Avatar
					...
					src={authContext.authState.userInfo ? `${authContext.authState.userInfo.image}` : ''}
					sx={{
						width: open ? 65 : 36,
						height: open ? 65 : 36,
					}}
				/>
				<Box sx={{ mt: open ? 0 : 1, position: open ? 'absolute' : 'static', right: 12, top: 10 }}>
				...
				>
					<MenuItem
						onClick={() => {
							history(`/${authContext.authState.userInfo.role}/profile/${authContext.authState.userInfo._id}`);
						}}
					>
						<Typography textAlign='center'>Profile Settings</Typography>
					</MenuItem>
					{Object.keys(authContext.authState.userInfo).length > 0 && <Divider />}
				...
				{open && (
					<Box sx={{ mt: 1.5 }}>
						<Typography variant='h6' component='span' sx={{ textTransform: 'capitalize', textAlign: 'center' }}>
							{authContext.authState.userInfo
								? `${authContext.authState.userInfo.firstName} ${authContext.authState.userInfo.lastName}`
								: ''}
						</Typography>
						<Typography variant='overline' component='span' sx={{ display: 'block', textAlign: 'center' }}>
							{authContext.authState.userInfo ? `${authContext.authState.userInfo.role}` : ''}
						</Typography>
					</Box>
				)}
			</Box>
			...
			<Box>
					...
					{menu.map((item, index) => {
						...
						let listItemProps = {
							component: forwardRef((props, ref) => (
								<Link ref={ref} {...props} to={`${routeConfig.basename}${item.url}`} />
							)),
						};
						return (
							<ListItem
								...
								sx={{
									display: 'block',
									backgroundColor: isCurrentRoute ? theme.palette.primary.main : 'transparent',
								}}
							>
								<ListItemButton
									{...listItemProps}
								...
								>
									...
									<ListItemText key={item.title} primary={item.title} sx={{ opacity: open ? 1 : 0 }} />
								</ListItemButton>
							</ListItem>
						);
					})}
				...
				{authContext.isAdmin() && (
					<>
						<List subheader={open && <ListSubheader component='div'>{dashboardUserLinks?.title}</ListSubheader>}>
						...
						<Divider />
						<List subheader={open && <ListSubheader component='div'>{dashboardBHLinks?.title}</ListSubheader>}>
				...
				{authContext.isLandlord() && (
					<>
						<List subheader={open && <ListSubheader component='div'>{landlordLinks?.title}</ListSubheader>}>
						...
						<Divider />
						<List subheader={open && <ListSubheader component='div'>{dashboardTenantLinks?.title}</ListSubheader>}>
				...
				>
					<IconButton onClick={() => setOpen(!open)}>{!open ? <ChevronRightIcon /> : <ChevronLeftIcon />}</IconButton>
				</Box>
			</Box>
		</Drawer>
	);
